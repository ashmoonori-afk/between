...
describe('Dashboard', () => {
  it('renders the broker-dominant layout with brand + phase', () => {
    const { lastFrame } = render(<Dashboard state={setPhase(base, 'developing', 'goal_locked')} events={[]} now="14:08:22" />)
  ...
  it('expands the approval bar at a human gate', () => {
    ...
    const events: BetweenEvent[] = [
      ...
      { v: 1, ts: '2026-06-19T14:08:20.000Z', cycle: 1, phase: 'human_gate', event: 'verify_passed' },
    ]
    const frame = render(<Dashboard state={human} events={events} now="14:08:22" />).lastFrame() ?? ''
