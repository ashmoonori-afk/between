...
External omo HEAVY review (`review.md`) also reconciled: Node `>=22.12` now unified across `package.json` + lockfile + README + DEVELOPMENT-PLAN + `tsup` target + CI (P1); **lint added to CI** + green (P1); `dash --interval` validated as integer ≥ 250ms + clamp + test (P2).
Deferred (documented): events.jsonl rotation (M1); full BetweenState zod schema (H3); esbuild dev-only LOW advisory — 0 prod vulns, no force-fix (L4); real `node-pty` transport (needs build-tools host); incremental split of `src/daemon/loop.ts` (~458 LOC) + `src/cli.ts` (~258 LOC) — under the 800 hard cap, tracked (review P2).
